...
namespace seal
{
    ...
    DecryptorTK::DecryptorTK(const EncryptionParameters &parms, const BigPoly &secret_key) :
    ...
    void DecryptorTK::decryptMU(const BigPolyArray &encrypted, BigPoly &destination, BigPoly &secretkeySPU, BigPoly &secretkeyH)
    {
        ...
        add_poly_poly_coeffmod(secret_key_.pointer(), secretkeySPU.pointer(), coeff_count, coeff_modulus_.pointer(), coeff_uint64_count, secret_key_.pointer());
        
        if (secret_key_==secretkeyH) {
    ...
    void DecryptorTK::compute_secret_key_array(int max_power)
    {
        if (max_power < 1)
        {
